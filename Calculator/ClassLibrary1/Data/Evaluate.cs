...
namespace CalculatorComponent.Data
{
    public class Evaluate
    {
        ...
        //main meathod
        public string Final_Res(string str)
        {
            ...
            for (int i = 0; i < str.Length; i++)
            {
                ...
                if (ch[i] != '+' && ch[i] != '*' && ch[i] != '/' && ch[i] != '%' && ch[i] != '(' && ch[i] != ')' && !flagSecondNum && flagFirstNum)
                {
                    ...
                    if (i + 1 < ch.Length && (ch[i + 1] == '-' || ch[i + 1] == '+' || ch[i + 1] == ')' || ch[i + 1] == '*' || ch[i + 1] == '/' || ch[i + 1] == '%'))
                    {
                        ...
                        flagSecondNum = true;
                        if (ContainsDecimalOrDivision(str))
                        {
                            if (double.TryParse(str2, out double result1))
                            {
                                //Console.WriteLine($"Parsed successfully: {result1}");
                                b = result1;
                            }
                            else
                            {
                                Console.WriteLine("Failed to parse the input as a double.");
                            }
                        }
                        else
                        {
                            if (int.TryParse(str2, out int result1))
                            {
                                //Console.WriteLine($"Parsed successfully: {result1}");
                                b = result1;
                            }
                            ...
                            {
                                Console.WriteLine("Failed to parse the input as a int.");
                            }
                ...
                {
                    //Console.WriteLine("This is called" + ch[i]);
                    str1 += ch[i];
                    ...
                    if (ch[i] != '-' && ch[i] != '+' && ch[i] != '*' && ch[i] != '/')
                    {
                        ...
                        if (ContainsDecimalOrDivision(str))
                        {
                            if (double.TryParse(str1, out double result1))
                            {
                                //Console.WriteLine($"Parsed successfully: {result1}");
                                a = result1;
                            }
                ...
                {
                    str1 += ch[i];
                    if (i+1<=ch.Length-1 && ch[i + 1] == '-' || ch[i + 1] == '+' || ch[i + 1] == '*' || ch[i + 1] == '/' || ch[i + 1] == '%')
                    {
                        flagFirstNum = true;
                        if (ContainsDecimalOrDivision(str))
                        {
        ...
        //check wheter to use int or double 
        static bool ContainsDecimalOrDivision(string input)
        {
